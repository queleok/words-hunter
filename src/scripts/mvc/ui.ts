...
class WordSynchronizer {
    ...
    public setLettersPublic(letters: string[]): void {
        const letters_div = document.getElementById('letters') as HTMLElement;
        letters_div.textContent = "";
    ...
    public highlightLetter(letter: string): boolean {
        this.word.splice(this.cursor++, 0, { letter: letter });
        if (!this.config.alphabet.includes(letter.toLowerCase())) return false;
    ...
    public dehighlightLetters(): void {
        for (const letter_widgets of this.letter_widgets) {
            if (!letter_widgets) continue;
    ...
    public getWord(): Array<WidgetPlaceholder> {
        return this.word;
    }
